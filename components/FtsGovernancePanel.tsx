    ...
    <div className="border border-rose-500/30 bg-zinc-900/40 p-3 flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between mb-2 border-b border-zinc-800 pb-1">
        <h2 className="font-black uppercase text-[10px] flex items-center gap-2">
          <Rocket size={12} className={fts.isTriggered ? "text-rose-500" : "text-amber-500"} />
          FTS: Flight_Termination_System
        </h2>
        <span className="text-[8px] opacity-40 uppercase tracking-widest">Range_Safety_Kernel</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className={`p-2 border ${fts.isTriggered ? 'border-rose-500 bg-rose-500/10' : 'border-zinc-800 bg-black/40'}`}>
          ...
          <div className={`text-xs font-black uppercase ${fts.isTriggered ? 'text-rose-500 animate-pulse' : fts.isArmed ? 'text-amber-500' : 'text-emerald-500'}`}>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar pr-1">
        ...
        <div>
          <div className="flex justify-between items-end mb-1">
            <div className="text-[7px] text-zinc-500 uppercase flex items-center gap-1">
              <Activity size={8} />
              V-Tube_Recoverability
            </div>
            <div className={`text-[9px] font-bold ${fts.recoverabilityScore < 0.3 ? 'text-rose-500' : fts.recoverabilityScore < 0.7 ? 'text-amber-500' : 'text-emerald-400'}`}>
        ...
        <div>
          <div className="flex justify-between items-end mb-1">
            ...
            <div className={`text-[9px] font-bold ${isOutside ? 'text-rose-500' : 'text-zinc-400'}`}>
        ...
        <div className="space-y-1">
          {fts.isTriggered && (
            <div className="p-1.5 bg-rose-500/10 border border-rose-500/30 text-[8px] text-rose-400 uppercase font-bold leading-tight">
              CRITICAL: {fts.terminationReason}
            </div>
          )}
          {fts.isRecoveryAttempted && !fts.isTriggered && (
            <div className="p-1.5 bg-amber-500/10 border border-amber-500/30 text-[8px] text-amber-400 uppercase font-bold leading-tight animate-pulse">
              WARNING: GOVERNED_RECOVERY_IN_PROGRESS
            </div>
          )}
          <div className="p-1.5 bg-zinc-800/30 text-[7px] text-zinc-500 uppercase leading-tight">
            STAGE: {stageStatus}
          </div>
        </div>
      </div>
      ...
      <div className="mt-2 pt-2 border-t border-zinc-800">
        <div className="flex justify-between items-center">
          <span className="text-[7px] text-zinc-500 uppercase tracking-tighter">L4_V-Tube_Projection</span>
