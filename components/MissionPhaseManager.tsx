    ...
    <div className="border border-[#00ff41]/20 bg-zinc-900/20 p-3 flex flex-col h-full">
      <div className="flex items-center justify-between mb-2 border-b border-zinc-800 pb-1">
        ...
        <div className="flex items-center gap-2">
           {isTransitioning && (
             <span className="text-[7px] bg-blue-900/30 text-blue-400 px-1 border border-blue-900 uppercase font-bold animate-pulse">Transition_Active</span>
           )}
           {isPreparing && !isTransitioning && (
             <span className="text-[7px] bg-amber-900/30 text-amber-500 px-1 border border-amber-900 uppercase font-bold animate-pulse">Preparing</span>
           )}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center gap-2">
        <div className="flex justify-between items-end">
          <div className="flex flex-col">
            <span className="text-[7px] text-zinc-500 uppercase">Current_Event</span>
            <span className="text-[10px] font-mono text-white">{state.currentEventId || 'INITIAL_FLIGHT'}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[7px] text-zinc-500 uppercase">Next_Event</span>
            <span className="text-[10px] font-mono text-[#00ff41]">{state.nextEventId || 'MISSION_END'}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-1">
          <div className="bg-black/40 border border-zinc-800 p-1.5 flex flex-col items-center">
            <span className="text-[6px] text-zinc-500 uppercase">τ_prepare</span>
            <span className="text-[10px] font-mono text-zinc-300">
              {tau_prepare.toFixed(0)}ms
            </span>
          </div>
          <div className="bg-black/40 border border-zinc-800 p-1.5 flex flex-col items-center">
            <span className="text-[6px] text-zinc-500 uppercase">T_minus</span>
            <span className="text-[10px] font-mono text-zinc-300">
      ...
      <div className="mt-2 flex flex-col gap-1 border-t border-zinc-800 pt-2">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[7px] uppercase ${isPreparing ? 'text-amber-400 font-bold' : 'text-zinc-500'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[7px] uppercase ${isTransitioning ? 'text-blue-400 font-bold' : 'text-zinc-500'}`}>
