  ...
  return (
    <div className={`border-l-4 p-4 rounded-r-lg transition-colors duration-500 ${getBorderColor()}`}>
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-sm font-black uppercase tracking-widest text-zinc-400">L9: Planner Interface</h4>
        <span className={`text-xs px-2 py-0.5 rounded-full font-bold border ${isSafe ? 'border-emerald-500/30' : 'border-rose-500/50'} ${getTextColor()}`}>
          {advisory.riskLevel}
        </span>
      </div>
      ...
      <div className="space-y-3">
        <div>
          <div className="flex justify-between items-end mb-1">
            <span className="text-[11px] text-zinc-500 uppercase">Rate-Limited Velocity Scale</span>
            <span className="text-[10px] opacity-30 uppercase">Target: {(advisory.targetVelocityScale * 100).toFixed(0)}%</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="mono text-xl font-bold">{(advisory.recommendedVelocityScale * 100).toFixed(0)}%</span>
          </div>
        </div>

        {advisory.anomalyDetected && (
          <div className="animate-pulse flex items-center gap-2 text-rose-500 text-xs font-bold uppercase tracking-tighter">
