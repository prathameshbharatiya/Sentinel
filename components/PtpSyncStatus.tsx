  ...
  return (
    <div className="border border-[#00ff41]/20 bg-zinc-900/20 p-3 flex flex-col h-full">
      <div className="flex items-center justify-between mb-2 border-b border-zinc-800 pb-1">
        ...
        <div className={`text-[10px] font-bold uppercase ${getQualityColor()}`}>
          {status.syncQuality}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center gap-3">
        <div className="flex flex-col items-center">
          <span className="text-[10px] text-zinc-500 uppercase mb-1">Clock_Offset</span>
          <div className={`text-2xl font-mono tracking-tighter ${getQualityColor()}`}>
            {status.offsetNs > 0 ? '+' : ''}{status.offsetNs.toFixed(1)}
            <span className="text-xs ml-1 opacity-60">ns</span>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-2">
          <div className="bg-black/40 border border-zinc-800 p-1.5 flex flex-col items-center">
            <span className="text-[10px] text-zinc-500 uppercase">Trust_Score</span>
            <span className={`text-xs font-bold ${status.isTrustworthy ? 'text-emerald-400' : 'text-rose-500'}`}>
              {status.isTrustworthy ? 'VERIFIED' : 'UNTRUSTED'}
            </span>
          </div>
          <div className="bg-black/40 border border-zinc-800 p-1.5 flex flex-col items-center">
            <span className="text-[10px] text-zinc-500 uppercase">Last_Sync</span>
      ...
      <div className="mt-2 flex items-center gap-2 border-t border-zinc-800 pt-2">
        ...
        <span className={`text-[10px] uppercase leading-tight ${status.isTrustworthy ? 'text-zinc-500' : 'text-rose-400 font-bold'}`}>
          {status.isTrustworthy 
