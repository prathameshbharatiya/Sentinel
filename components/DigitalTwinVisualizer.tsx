    ...
    <div className="border border-[#00ff41]/30 bg-zinc-900/40 p-3 flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between mb-2 border-b border-zinc-800 pb-1">
        <h2 className="font-black uppercase text-xs flex items-center gap-2">
          <Layers size={14} className="text-[#00ff41] animate-pulse" />
          L2: Digital_Twin_Observer
        </h2>
        <div className="flex items-center gap-2">
          {state.isMultiBody && (
            <span className="text-[9px] bg-[#00ff41]/20 text-[#00ff41] px-1 border border-[#00ff41]/30 uppercase font-bold">Multi_Body</span>
          )}
          <span className="text-[10px] opacity-40 uppercase tracking-widest">Active_Sync</span>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-3 shrink-0">
        <div className="space-y-0.5">
          <span className="text-[9px] text-zinc-500 uppercase">Total_Mass</span>
          <div className="text-lg font-black text-white leading-none">{state.totalMass.toFixed(3)}<span className="text-[11px] text-zinc-600 ml-1">kg</span></div>
        </div>
        <div className="space-y-0.5">
          <span className="text-[9px] text-zinc-500 uppercase">Stability</span>
          <div className={`text-lg font-black leading-none ${state.stabilityMargin < 0.3 ? 'text-rose-500' : 'text-[#00ff41]'}`}>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar pr-1">
        ...
        {state.aero && (
          <div className="bg-zinc-950/60 border border-zinc-800 p-2 space-y-2">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-black uppercase text-blue-400">Aero_Dynamics (Physics-Informed)</span>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-0.5">
                <span className="text-[10px] text-zinc-500 uppercase">Mach_Number</span>
                <div className={`text-xs font-mono font-bold ${state.aero.machNumber > 0.8 ? 'text-amber-400' : 'text-white'}`}>
                  M {state.aero.machNumber.toFixed(3)}
                </div>
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] text-zinc-500 uppercase">Drag_Coeff (Cd)</span>
                <div className="text-xs font-mono font-bold text-white">
                  {state.aero.dragCoefficient.toFixed(3)}
                </div>
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] text-zinc-500 uppercase">Air_Density (ρ)</span>
                <div className="text-xs font-mono font-bold text-white">
                  {state.aero.atmosphericDensity.toFixed(4)} <span className="text-[10px] opacity-40">kg/m³</span>
                </div>
              </div>
              <div className="space-y-0.5">
                <span className="text-[10px] text-zinc-500 uppercase">Altitude</span>
                <div className="text-xs font-mono font-bold text-white">
                  {state.aero.altitude.toFixed(1)} <span className="text-[10px] opacity-40">m</span>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-1">
          <span className="text-[9px] text-zinc-500 uppercase block mb-1">Body_Registry</span>
            ...
            {state.bodies.map(body => (
              <div key={body.id} className="bg-black/40 border border-zinc-800 p-1.5 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-mono text-zinc-300">{body.id}</span>
                </div>
                <div className="flex gap-3">
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] text-zinc-600 uppercase">Mass</span>
                    <span className="text-xs font-mono text-white">{body.mass.toFixed(2)}</span>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] text-zinc-600 uppercase">Cov</span>
                    <span className="text-xs font-mono text-amber-500">{body.covariance.toFixed(0)}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="mt-3 pt-2 border-t border-zinc-800 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1">
            <div className="text-[10px] font-bold text-white uppercase leading-none">{topology}</div>
            <div className="text-[9px] text-zinc-500 uppercase tracking-tighter">Physics_Engine_Synced</div>
          </div>
        </div>
      </div>
    </div>
